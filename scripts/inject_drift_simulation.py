...
Usage   : python scripts/inject_drift_simulation.py [--scenario A|B|C|D]
...
Résultats visibles dans Grafana (http://localhost:3000) en < 8 minutes.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        help="Scénario de dérive : A=abrupt, B=graduel (défaut), C=cyclique, D=incrémental"
    )
    ...
    log.info(f"  Scénario sélectionné : {args.scenario}")
    log.info("  Résultats visibles dans Grafana → http://localhost:3000")
    log.info("  Tableau de bord Streamlit → http://localhost:8501")
    log.info("="*60)

    scenarios = {
        "A": scenario_a_abrupt,
        "B": scenario_b_gradual,
        "C": scenario_c_cyclic,
        "D": scenario_d_incremental,
    }
    scenarios[args.scenario](producer)

    log.info("")
    log.info("✅ Injection terminée.")
    log.info("   → Grafana : http://localhost:3000  (Score Composite Drift + Alertes)")
    log.info("   → Streamlit : http://localhost:8501  (page Drift & Apprentissage)")
